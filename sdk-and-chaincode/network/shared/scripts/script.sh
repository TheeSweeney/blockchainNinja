...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	execute "peer chaincode install -n mycc -v 1.0 -l node -p /node-chaincode" >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		execute "peer chaincode instantiate -o $ORDERER_ADDRESS -C $CHANNEL_NAME -n mycc -v 1.0 -c '$c' -P \"OR	('$ORG1_MSP.member','$ORG2_MSP.member')\"" >&log.txt
	else
		execute "peer chaincode instantiate -o $ORDERER_ADDRESS --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -v 1.0 -c '$c' -P \"OR	('$ORG1_MSP.member','$ORG2_MSP.member')\"" >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     execute "peer chaincode query -C $CHANNEL_NAME -n mycc -c '$c'" >&log.txt
...
chaincodeInvoke () {
	...
	# lets supply it directly as we know it using the "-o" option
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		execute "peer chaincode invoke -o $ORDERER_ADDRESS -C $CHANNEL_NAME -n mycc -c '$c'" >&log.txt
	else
		execute "peer chaincode invoke -o $ORDERER_ADDRESS --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '$c'" >&log.txt
	fi
	res=$?
	cat log.txt
...
instantiateChaincode 2 '{"Args":["unused"]}'

sleep 5

echo "Creating initial data"
chaincodeInvoke 2 '{"Args":["initMarble","marble1","blue","35","tom"]}'

#Query on chaincode on Peer0/Org1
echo "Querying chaincode on org1/peer0..."
chaincodeQuery 0 '{"Args":["readMarble","marble1"]}' '{"color":"blue","docType":"marble","name":"marble1","owner":"tom","size":35}'
...
#Complex query on chaincode on Peer3/Org2 (using couchdb)
echo "Querying chaincode on org2/peer3..."
chaincodeQuery 3 '{"Args":["queryMarblesByOwner","jerry"]}' '[{"Key":"marble1","Record":{"color":"blue","docType":"marble","name":"marble1","owner":"jerry","size":35}}]'
